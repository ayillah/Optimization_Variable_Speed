    ...
    *) history -- list of (time, solution) tuples filled in during the call to
    ...
    def run(self, t_init, t_final, u_init):
        ...
        while t < t_final:
            ...
            all_data.append(deepcopy(u_cur_interp))
        ...
        all_data_array += 0.001 * np.random.randn(*all_data_array.shape)

        # Save the adjoint solution data
        np.save(f'StateEquation_SolutionsStar_nx_{nx}.npy', all_data_array)

        #np.save(f'StateEquation_Solutions_nx_{nx}.npy', all_data_array)
...
if __name__ == "__main__":
    ...
    for nx in (16, 32, 64, 128):
        grid = Grid1D(nx=nx)
        speed_func = InterpolatedSpeedFunction(nx=nx, 
                                               speed_to_interpolate=base_speed_func)
        ...
        for i, v in enumerate(stepper.history):
            ...
            if doPlots:
                ...
                ax.plot(grid.X, Wex, 'r-', label='u_extact')
                ax.set(ylim=[-4,4])
                ax.legend()
