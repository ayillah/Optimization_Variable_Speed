...
class MacCormackInterpolationAligned:
    ...
    def run_interpolation(self):
        ...
        for nx in self.nx_values:
            ...
            # Perform interpolation for solutions
            ...
            #np.save(f"aligned_interpolated_interp_solutions_star_nx_{nx}.npy", interpolated_array)

            # Perturb the solution
            interpolated_solutions = np.array(interpolated_solutions)
            interpolated_solutions += 0.001 * np.random.randn(*interpolated_solutions.shape)
            np.save(f"aligned_interpolated_interp_solutions_star_nx_{nx}.npy", interpolated_solutions)
    ...
    def generate_plots(self, grid, interpolated_solutions, original_solutions, nx, transformed_times, target_times):
        ...
        for i, t in enumerate(target_times):
            ...
            plt.plot(grid.X, interpolated_u[0, :], "o", label=f"Interpolated Solution at t={t:.7f}", color='g')
            plt.xlabel("X")
